...
export const Logo: React.FC<LogoProps> = ({ className = "", theme = 'day' }) => {
  ...
  return (
    <div className={`inline-flex items-center justify-center gap-3 select-none font-black w-fit mx-auto ${className} ${textColor}`}>
      ...
      <span className="text-2xl md:text-4xl tracking-tight leading-none">معلم</span>
      
      {/* Centered Plus Sign - Using a box to force geometry */}
      <div className={`flex items-center justify-center h-8 w-8 transition-colors duration-500 ${accentColor}`}>
        <span className="text-4xl md:text-5xl mb-2">+</span>
      </div>
      
      {/* English Word - Left side in RTL */}
      <span className={`text-2xl md:text-4xl tracking-tighter transition-colors duration-500 font-sans leading-none ${accentColor}`}>
        Plus
      </span>
    </div>
  );
